...
def inference(cfg, epoch, model, dataloader, device, is_titan, logger=None, eval_kde_nll=False, test_mode=False):
    model.eval()
    all_img_paths = []
    all_X_globals = []
    all_pred_goals = []
    ...
    with torch.set_grad_enabled(False):
        ...
        eval_results, MSE_dict, per_step_box_mse, per_step_center_mse = evaluate_multimodal(all_pred_trajs, all_gt_trajs, is_titan, mode=mode, distribution=distribution, bbox_type=cfg.DATASET.BBOX_TYPE, logger = logger)
        ...
        if test_mode:
            ...
            # Dump different genders
            # print(os.path.join(cfg.OUT_DIR, f'BiTrapD_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.GENDER}_{cfg.TEST.AGE}.pkl'))
            output_file = os.path.join(cfg.OUT_DIR, f'BiTrapNP/BiTrapNP_{cfg.DATASET.NAME}_{cfg.TEST.AGE}_{cfg.TEST.SPLIT}.pkl')
            # output_file = os.path.join(cfg.OUT_DIR, f'BiTrapNP_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.GENDER}_{cfg.TEST.AGE}.pkl')
            print("Writing outputs to: ", output_file)
            pkl.dump(MSE_dict, open(output_file, 'wb'))

            # Dump different ages
            # Dump error metrics
            # print(os.path.join(cfg.OUT_DIR, f'BiTrapD_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.AGE}.pkl'))
            # output_file = os.path.join(cfg.OUT_DIR, f'BiTrapNP_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.AGE}.pkl')
            # print("Writing outputs to: ", output_file)
            # Dump evaluation results
            # pkl.dump(MSE_dict, open(output_file,'wb'))

            # Dump raw error data
            # print(os.path.join(cfg.OUT_DIR, f'BiTrapD_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.GENDER}_raw.pkl'))
            # output_file = os.path.join(cfg.OUT_DIR, f'BiTrapD_{cfg.DATASET.NAME}_{cfg.TEST.SPLIT}_{cfg.TEST.GENDER}_raw.pkl')
            # print('Writing outputs to:', output_file)
            # raw_dict = {'Box': per_step_box_mse, 'Center': per_step_center_mse}
            # # pkl.dump(raw_dict, open(output_file,'wb'))
            # print('Done!')
