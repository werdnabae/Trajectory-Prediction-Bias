...
def evaluate_multimodal(pred_traj, target_traj, is_titan, mode='bbox', distribution=None, bbox_type='x1y1x2y2', logger=None):
    '''
    ...
    if mode == 'bbox':
        eval_results, MSE_dict, per_step_box_mse, per_step_center_mse = evaluate_bbox_traj(pred_traj, tiled_target_traj, is_titan, bbox_type=bbox_type, logger=logger)
    elif mode == 'point':
        eval_results = {}
        ...
        eval_results['per_step_displacement_error'] = np.linalg.norm(pred_traj - tiled_target_traj, axis=-1).min(
            axis=2).mean(axis=0)
    else:
            ...
            if target_traj.shape[-1] == 4:
                if bbox_type == 'x1y1x2y2':
                    eval_results['mode_ADE'] = np.square(mode_traj - \
                                                         target_traj[:, :, None, :]).mean(axis=(1, 3)).min(
                        axis=-1).mean()
                    eval_results['mode_CADE'] = np.square(x1y1x2y2_to_cxcywh(mode_traj)[..., :2] - \
                                                          x1y1x2y2_to_cxcywh(target_traj)[:, :, None, :2]).mean(
                        axis=(1, 3)).min(axis=-1).mean()
                    ...
                    eval_results['mode_CFDE'] = np.square(x1y1x2y2_to_cxcywh(mode_traj)[:, -1, :, :2] - \
                                                          x1y1x2y2_to_cxcywh(target_traj)[:, -1, None, :2]).mean(
                        axis=-1).min(axis=-1).mean()
                elif bbox_type == 'cxcywh':
                    eval_results['mode_ADE'] = np.square(cxcywh_to_x1y1x2y2(mode_traj) - \
                                                         cxcywh_to_x1y1x2y2(target_traj)[:, :, None, :]).mean(
                        axis=(1, 3)).min(axis=-1).mean()
                    eval_results['mode_CADE'] = np.square(mode_traj[..., :2] - \
                                                          target_traj[:, :, None, :2]).mean(axis=(1, 3)).min(
                        axis=-1).mean()
                    eval_results['mode_FDE'] = np.square(cxcywh_to_x1y1x2y2(mode_traj)[:, -1] - \
                                                         cxcywh_to_x1y1x2y2(target_traj)[:, -1, None, :]).mean(
                        axis=-1).min(axis=-1).mean()
                    eval_results['mode_CFDE'] = np.square(mode_traj[:, -1, :, :2] - \
                                                          target_traj[:, -1, None, :2]).mean(axis=-1).min(
                        axis=-1).mean()

    return eval_results, MSE_dict, per_step_box_mse, per_step_center_mse


def evaluate_bbox_traj(pred_traj, target_traj, is_titan, bbox_type='x1y1x2y2', logger=None):
    '''
    ...
    if is_titan: # titan has is annotated less frequenly, so we need to adjust how we calculate our error values
        ade_05 = per_step_box_mse[:, 0:5].mean(axis=(1, 3)).min(axis=1)
        eval_results['ADE(0.5s)'] = ade_05.mean()

        ade_10 = per_step_box_mse[:, 0:10].mean(axis=(1, 3)).min(axis=1)
        eval_results['ADE(1.0s)'] = ade_10.mean()
    ...
    else:  # if the dataset used is not TITAN
        # NOTE: mean over dim and time, then min over K samples, then mean over batch
        ade_05 = per_step_box_mse[:, 0:15].mean(axis=(1, 3)).min(axis=1)
        eval_results['ADE(0.5s)'] = ade_05.mean()
        # eval_results['ADE(0.5s)_SD'] = np.std(ade_05)
        # eval_results['ADE(0.5s)'] = per_step_box_mse[:, 0:15].mean(axis=(1, 3)).min(axis=1).mean()
        ade_10 = per_step_box_mse[:, 0:30].mean(axis=(1, 3)).min(axis=1)
        eval_results['ADE(1.0s)'] = ade_10.mean()
        # eval_results['ADE(1.0s)_SD'] = np.std(ade_10)
        # eval_results['ADE(1.0s)'] = per_step_box_mse[:, 0:30].mean(axis=(1, 3)).min(axis=1).mean()
        mse_45 = per_step_box_mse.mean(axis=(1, 3)).min(axis=1)
        eval_results['ADE(1.5s)'] = mse_45.mean()
        # eval_results['ADE(1.5s)_SD'] = np.std(mse_45)
        mse_last = per_step_box_mse[:, -1].mean(axis=-1).min(axis=1)
        #eval_results['FDE'] = mse_last.mean()
        #eval_results['FDE_SD'] = np.std(mse_last)



        c_ade_05 = per_step_center_mse[:, 0:15].mean(axis=(1, 3)).min(axis=1)
        #eval_results['C-ADE(0.5s)'] = c_ade_05.mean()
        #eval_results['C-ADE(0.5s)_SD'] = np.std(c_ade_05)
        # eval_results['C-ADE(0.5s)'] = per_step_center_mse[:, 0:15].mean(axis=(1, 3)).min(axis=1).mean()
        c_ade_10 = per_step_center_mse[:, 0:30].mean(axis=(1, 3)).min(axis=1)
        #eval_results['C-ADE(1.0s)'] = c_ade_10.mean()
        #eval_results['C-ADE(1.0s)_SD'] = np.std(c_ade_10)
        # eval_results['C-ADE(1.0s)'] = per_step_center_mse[:, 0:30].mean(axis=(1, 3)).min(axis=1).mean()
        cmse_45 = per_step_center_mse.mean(axis=(1, 3)).min(axis=1)
        eval_results['C-ADE(1.5s)'] = cmse_45.mean()
        #eval_results['C-ADE(1.5s)_SD'] = np.std(cmse_45)
        cmse_last = per_step_center_mse[:, -1].mean(axis=-1).min(axis=1)
        eval_results['C-FDE'] = cmse_last.mean()
        #eval_results['C-FDE_SD'] = np.std(cmse_last)

    # Create graphs
    # fig, ax = plt.subplots(figsize=(6, 4))
    # ax.hist(ade_05, rwidth=0)
    # ax.set_title('ADE(0.5)')
    # ax.set_xlabel('MSE')
    # ax.set_ylabel('# of trajectories')
    # logger.log_plot(fig, 'ADE(0.5)')
    # fig.clf()
    #
    # fig, ax = plt.subplots(figsize=(6, 4))
    # ax.hist(ade_10, rwidth=0)
    # ax.set_title('ADE(1.0)')
    # ax.set_xlabel('MSE')
    # ax.set_ylabel('# of trajectories')
    # logger.log_plot(fig, 'ADE(1.0)')
    # fig.clf()
    #
    # fig, ax = plt.subplots(figsize=(6, 4))
    # ax.hist(mse_45, rwidth=0)
    # ax.set_title('ADE(1.5)')
    # ax.set_xlabel('MSE')
    # ax.set_ylabel('# of trajectories')
    # logger.log_plot(fig, 'ADE(1.5)')
    # fig.clf()
    #
    # fig, ax = plt.subplots(figsize=(6, 4))
    # ax.hist(cmse_45, rwidth=0)
    # ax.set_title('C-ADE(1.5)')
    # ax.set_xlabel('MSE')
    # ax.set_ylabel('# of trajectories')
    # logger.log_plot(fig, 'C-ADE(1.5)')
    # fig.clf()
    #
    # fig, ax = plt.subplots(figsize=(6, 4))
    # ax.hist(cmse_last, rwidth=0)
    # ax.set_title('C-FDE(1.5)')
    # ax.set_xlabel('MSE')
    # ax.set_ylabel('# of trajectories')
    # logger.log_plot(fig, 'C-FDE(1.5)')
    # fig.clf()

    MSE_dict = {'MSE_05': ade_05, 'MSE_10': ade_10, 'MSE_15': mse_45,
                'CMSE': cmse_45, 'CFMSE': cmse_last}
