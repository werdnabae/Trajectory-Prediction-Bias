...
class BiTraPGMM(nn.Module):
    def __init__(self, cfg, dataset_name=None):
        ...
        #self.node_future_encoder_h = nn.Linear(6, 32) # what is was originally
        self.node_future_encoder_h = nn.Linear(self.cfg.GLOBAL_INPUT_DIM, 32) # what I changed
    ...
    def pred_future_traj_GMM(self, dec_h, goal_loc_gmm, K=25):
        ...
        # print()
        # print(goal_loc_gmm.input_log_pis.shape)
        # print(torch.reshape(goal_loc_gmm.input_log_pis, [-1, 1]).shape)
        # print()
        # print(goal_loc_gmm.mus.shape)
        # print(torch.reshape(goal_loc_gmm.mus, [-1, 1, 2]).shape)
        # print()
        # print(goal_loc_gmm.log_sigmas.shape)
        # print(torch.reshape(goal_loc_gmm.log_sigmas, [-1, 1, 2]).shape)
        # print()
        # print(goal_loc_gmm.corrs.shape)
        # print(torch.reshape(goal_loc_gmm.corrs, [-1, 1, 1]).shape)
        # this fixes the problem I was having in gmm4d
        # flatten_goal_dist = self.GMM(torch.reshape(goal_loc_gmm.input_log_pis, [-1, 2]),
        #                              torch.reshape(goal_loc_gmm.mus, [-1, 2, 4]),
        #                              torch.reshape(goal_loc_gmm.log_sigmas, [-1, 2, 4]),
        #                              torch.reshape(goal_loc_gmm.corrs, [-1, 2, 2]))
        ...
        # print(dec_h.view(-1, dec_h.shape[-1]).shape)
        # print(self.traj_dec_input_backward(inv_loc_mus).shape)
        # backward_input = torch.cat([dec_h.view(-1, dec_h.shape[-1]), torch.reshape(self.traj_dec_input_backward(inv_loc_mus), [-1, 2])], dim=-1)
