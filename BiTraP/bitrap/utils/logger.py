...
class Logger(logging.Logger):
    ...
    def __init__(self, name, config, project, viz_backend=None, sync=True):
        ...
        self.run_id = f'BiTrap-D {config.DATASET.NAME} {config.TEST.AGE} {config.TEST.SPLIT}'
...
class _WandBBackend(_Backend):
    ...
    def __init__(self, config, project, run_id, sync=True):

        os.environ['WANDB_MODE'] = 'run' if sync else 'dryrun'
        wandb.init(config=config, project=project)
        wandb.run.name = f'BiTrap-D {config.DATASET.NAME} {config.TEST.AGE} {config.TEST.SPLIT}'
