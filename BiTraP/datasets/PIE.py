...
class PIEDataset(data.Dataset):
    def __init__(self, cfg, split):
        self.split = split  # Indicates what split of data to provide (train, val, test)
        ...
        traj_model_opts = {'normalize_bbox': True,
                       'track_overlap': 0.5, # How much the sampled tracks should overlap. A value between [0,1) should be selected
                       'observe_length': 15,
                       'predict_length': 45,
                       'enc_input_type': ['bbox'],
                       'dec_input_type': [], 
        ...
        imdb = PIE(data_path=self.root)  # What is this?
        ...
        beh_seq = imdb.generate_data_trajectory_sequence(self.split, cfg.TEST.AGE, cfg.TEST.GENDER, **traj_data_opts) #  The trajectories are not split into two 0.5s and 1.5s sequences yet
