...
from datasets.TITAN_origin import TITAN
...
class TITANDataset(data.Dataset):
    def __init__(self, cfg, split):  # need to add cfg to this later
        self.split = split
        self.root = '/home/anbae/Documents/Research/Datasets/TITAN'  # cfg.DATASET.ROOT
        self.cfg = cfg
        ...
        self.downsample_step = int(10 / self.cfg.DATASET.FPS)
        imdb = TITAN(data_path=self.root)
        beh_seq = imdb.generate_data_trajectory_sequence(self.split, cfg.TEST.AGE, **data_opts)
    ...
    def __getitem__(self, index):
        ...
        ret = {'input_x': obs_bbox, 'target_y': pred_bbox, 'pred_resolution': pred_resolution}
        return ret
