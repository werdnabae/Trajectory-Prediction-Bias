...
from configs.titan import parse_sgnet_args as parse_args
from lib.models import build_model
from lib.losses import rmse_loss
from lib.utils.titan_train_utils import train, val, test


def main(args):
    this_dir = osp.dirname(__file__)
    model_name = args.model
    save_dir = osp.join(this_dir, 'checkpoints', model_name, 'titan_transfer_pie')
    ...
    if osp.isfile(args.checkpoint):
        checkpoint = torch.load(args.checkpoint, map_location=device)
        if checkpoint['model_state_dict'] is not None:  # Uses this for the checkpoint provided by SGNet authors
            model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        else:  # Uses this for checkpoints trained using our code
    ...
    for epoch in range(args.start_epoch, args.epochs + args.start_epoch):
        ...
        # print('saving to: ', os.path.join(save_dir, 'Epoch_{}.pth'.format(str(epoch).zfill(3))))
        ...
        print("MSE_05: %4f;  MSE_10: %4f;  MSE_15: %4f\n" % (MSE_05, MSE_10, MSE_15))
        print('saving to: ', os.path.join(save_dir, 'Epoch_{}.pth'.format(str(epoch).zfill(3))))
        torch.save(model.module.state_dict(), os.path.join(save_dir, 'Epoch_{}.pth'.format(str(epoch).zfill(3))))
