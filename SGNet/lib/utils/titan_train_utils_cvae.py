...
from lib.utils.eval_utils import eval_titan_cvae, eval_jaad_pie_cvae
...
def val(model, val_gen, criterion, device):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):
            ...
            all_goal_traj, cvae_dec_traj, KLD_loss, _ = model(inputs=input_traj, map_mask=None, targets=None,
                                                              training=False)
            cvae_loss = cvae_multi(cvae_dec_traj, target_traj)
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):
            ...
            batch_MSE_15, batch_MSE_05, batch_MSE_10, batch_FMSE, batch_CMSE, batch_CFMSE, batch_FIOU = \
                eval_titan_cvae(input_traj_np, target_traj_np[:, -1, :, :], cvae_dec_traj[:, -1, :, :, :])
            MSE_15 += batch_MSE_15
            MSE_05 += batch_MSE_05
            MSE_10 += batch_MSE_10
            FMSE += batch_FMSE
