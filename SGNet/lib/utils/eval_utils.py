...
def eval_jaad_pie(input_traj_np, target_traj_np, all_dec_traj_np):
    ...
    for batch_index in range(all_dec_traj_np.shape[0]):
        ...
        batch_FIOU += np.mean(tmp_FIOU)

    MSE_dict = {'MSE_05': all_MSE_05, 'MSE_10': all_MSE_10, 'MSE_15': all_MSE_15,
                'CMSE': all_CMSE, 'CFMSE': all_CFMSE}

    return batch_MSE_15, batch_MSE_05, batch_MSE_10, batch_FMSE, batch_CMSE, batch_CFMSE, batch_FIOU, MSE_dict
...
def eval_jaad_pie_cvae(input_traj, target_traj, cvae_all_dec_traj):
    ...
    MSE_05 = np.square(cvae_all_dec_traj_xyxy[:, :15, :, :] - tiled_target_traj_xyxy[:, :15, :, :]).mean(
        axis=(1, 3)).min(axis=-1).sum()
    # import pdb; pdb.set_trace()
    MSE_10 = np.square(cvae_all_dec_traj_xyxy[:, :30, :, :] - tiled_target_traj_xyxy[:, :30, :, :]).mean(
        axis=(1, 3)).min(axis=-1).sum()
    MSE_15 = np.square(cvae_all_dec_traj_xyxy - tiled_target_traj_xyxy).mean(axis=(1, 3)).min(axis=-1).sum()
    FMSE = np.square(cvae_all_dec_traj_xyxy[:, -1, :, :] - tiled_target_traj_xyxy[:, -1, :, :]).mean(axis=-1).min(
        axis=-1).sum()
    CMSE = np.square(cvae_all_dec_traj[:, :, :, :2] - tiled_target_traj[:, :, :, :2]).mean(axis=(1, 3)).min(
        axis=-1).sum()
    CFMSE = np.square(cvae_all_dec_traj[:, -1, :, :2] - tiled_target_traj[:, -1, :, :2]).mean(axis=-1).min(
        axis=-1).sum()
    return MSE_15, MSE_05, MSE_10, FMSE, CMSE, CFMSE, FIOU


def eval_titan(input_traj_np, target_traj_np, all_dec_traj_np):
    batch_MSE_15 = 0
    batch_MSE_05 = 0
    batch_MSE_10 = 0
    batch_FMSE = 0
    ...
    for batch_index in range(all_dec_traj_np.shape[0]):
        ...
        MSE_15 = np.square(target_traj_xyxy[-1, 0:15, :] - all_dec_traj_xyxy[-1, 0:15, :]).mean(axis=None)
        all_MSE_15.append(MSE_15)
        batch_MSE_15 += MSE_15

        MSE_05 = np.square(target_traj_xyxy[-1, 0:5, :] - all_dec_traj_xyxy[-1, 0:5, :]).mean(axis=None)
        all_MSE_05.append(MSE_05)
        batch_MSE_05 += MSE_05

        MSE_10 = np.square(target_traj_xyxy[-1, 0:10, :] - all_dec_traj_xyxy[-1, 0:10, :]).mean(axis=None)
        all_MSE_10.append(MSE_10)
        batch_MSE_10 += MSE_10

        FMSE = np.square(target_traj_xyxy[-1, 14, :] - all_dec_traj_xyxy[-1, 14, :]).mean(axis=None)
        batch_FMSE += FMSE

        CMSE = np.square(target_traj[-1, 0:15, :2] - all_dec_traj[-1, 0:15, :2]).mean(axis=None)
        all_CMSE.append(CMSE)
        batch_CMSE += CMSE

        CFMSE = np.square(target_traj[-1, 14, :2] - all_dec_traj[-1, 14, :2]).mean(axis=None)
        all_CFMSE.append(CFMSE)
        batch_CFMSE += CFMSE

        tmp_FIOU = []
        for i in range(target_traj_xyxy.shape[0]):
            tmp_FIOU.append(compute_IOU(target_traj_xyxy[i, 14, :], all_dec_traj_xyxy[i, 14, :], format='x1y1x2y2'))
...
def eval_titan_cvae(input_traj, target_traj, cvae_all_dec_traj):
    MSE_15 = 0
    MSE_05 = 0
    MSE_10 = 0
    FMSE = 0
    ...
    MSE_05 = np.square(cvae_all_dec_traj_xyxy[:, :5, :, :] - tiled_target_traj_xyxy[:, :5, :, :]).mean(
        axis=(1, 3)).min(axis=-1).sum()
    # import pdb; pdb.set_trace()
    MSE_10 = np.square(cvae_all_dec_traj_xyxy[:, :10, :, :] - tiled_target_traj_xyxy[:, :10, :, :]).mean(
        axis=(1, 3)).min(axis=-1).sum()
    MSE_15 = np.square(cvae_all_dec_traj_xyxy - tiled_target_traj_xyxy).mean(axis=(1, 3)).min(axis=-1).sum()
    FMSE = np.square(cvae_all_dec_traj_xyxy[:, -1, :, :] - tiled_target_traj_xyxy[:, -1, :, :]).mean(axis=-1).min(
        axis=-1).sum()
...
def eval_hevi(input_traj_np, target_traj_np, all_dec_traj_np):
    ...
    for batch_index in range(all_dec_traj_np.shape[0]):
        ...
        ADE_10 += np.mean(
            np.sqrt(np.sum((target_traj_xyxy[:, 0:10, :2] - all_dec_traj_xyxy[:, 0:10, :2]) ** 2, axis=-1)))
...
def eval_ethucy_cvae(input_traj, target_traj, cvae_all_traj):
    ...
    result['ADE_08'] = np.linalg.norm(cvae_all_traj[:, :8, :, :] - tiled_target_traj[:, :8, :, :], axis=-1).mean(
        axis=1).min(axis=1).sum()
    result['ADE_12'] = np.linalg.norm(cvae_all_traj[:, :12, :, :] - tiled_target_traj[:, :12, :, :], axis=-1).mean(
        axis=1).min(axis=1).sum()
    result['FDE_08'] = np.linalg.norm(cvae_all_traj[:, 7, :, :] - tiled_target_traj[:, 7, :, :], axis=-1).min(
        axis=1).sum()
    result['FDE_12'] = np.linalg.norm(cvae_all_traj[:, 11, :, :] - tiled_target_traj[:, 11, :, :], axis=-1).min(
        axis=1).sum()
...
def eval_nuscenes_local(starting_translation, starting_rotation, target_traj, cvae_all_traj):
    ...
    for k in range(K):
        for b in range(B):
            cvae_all_traj_global[b, :, k, :] = convert_local_coords_to_global(cvae_all_traj[b, :, k, :],
                                                                              starting_translation[b],
                                                                              starting_rotation[b])
    result['ADE_12'] = np.linalg.norm(cvae_all_traj_global[:, :12, :, :] - tiled_target_traj[:, :12, :, :],
                                      axis=-1).mean(axis=1).min(axis=1).sum()
    result['FDE_12'] = np.linalg.norm(cvae_all_traj_global[:, 11, :, :] - tiled_target_traj[:, 11, :, :], axis=-1).min(
        axis=1).sum()
